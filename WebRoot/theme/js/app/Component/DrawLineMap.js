 ...
 * 绘制路径显示地图
 */
Ext.define('MyApp.Component.DrawLineMap',{
   ...
   constructor: function(){
      ...
      this.drawLine = function(path) {
          ...
          for(var i=0; i < ptStrArr.length-1;i++) {
                ...
                if(i>0){
                	var xx = ptStrArr[i-1].split(',')[0];
                var yy = ptStrArr[i-1].split(',')[1];
                 var dist = map.getDistance(new BMap.Point(x,y),new BMap.Point(xx,yy));
                 if(dist<10){
