 ...
 */
Ext.define('MyApp.Component.DriverTracePlayer',{
    ...
    constructor: function() {  
        ...
        //var map = new BMap.Map(this.mapId);
          var map = new BMap.Map(this.mapId,{minZoom:4,maxZoom:16}); 
    ...
    load : function(params,fn) {
        var player = this;
        Ext.ajaxModelLoader('MyApp.Model.DriverDateTrace',{
            params:params,
            success:function(data){
                player.atrace = packData(data);
                player.polylines = drawTrace.call(player,player.atrace);
                if(player.polylines.length) fn.call(player);
            }
        }).request();
        
        //上一个位置点,当天当前里程，当天行驶时间
        var lastgps = null,plate=null,curmileage = 0,drivetime = 0;
        
        function packData(data) {
            var atrace = [], btrace = [], gps = null,t = 0;
            if(data.length > 1)
            while((gps = data.shift())){
                var bgps = [new BMap.Point(gps[0],gps[1]),gps[2],gps[3],gps[4],gps[5]];
                var gpsDriveInfo = getGPSDriveInfo(bgps);
                var dt = new Date(gps[2]);
                if(!data.length || (t && dt.getTime() - t > 180000)) {
                    //上报时间超过三分钟则分段
                    atrace.push(btrace);   
                    btrace = [];
                }
        ...
        function drawTrace(atrace) {
            ...
            if(atrace[0] &&atrace[0][0]) setTimeout(function(){map.centerAndZoom(atrace[0][0][0],13);},5);
    ...
    animate: function() {
        ...
        if(tick <= player.slider.maxValue){
            var gps = MyApp.Component.DriverTracePlayer.getGPS.call(player,tick);
    ...
    getTickTime:function(tick) {
        var gps = MyApp.Component.DriverTracePlayer.getGPS.call(this,tick);
        return gps[1];
    }
});
