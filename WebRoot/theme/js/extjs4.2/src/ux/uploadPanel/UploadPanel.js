...
Ext.define('Ext.ux.uploadPanel.UploadPanel',{
	...
	columns : [
        //{xtype: 'rownumberer'},
		{text: '文件名', width:'16%',dataIndex: 'name'},
		{text: '自定义文件名', width: '16%',dataIndex: 'fileName',editor: {xtype: 'textfield'}},
        {text: '类型', width: '10%',dataIndex: 'type'},
        {text: '大小', width: '10%',dataIndex: 'size',renderer:function(v){
        	return Ext.util.Format.fileSize(v);
        }},
        {text: '进度', width: '19%',dataIndex: 'percent',renderer:function(v){        	
			var stml =
				'<div>'+
					'<div style="border:1px solid #008000;height:10px;width:100%;margin:2px 0px 1px 0px;float:left;">'+		
        ...
        {text: '状态', width: '13%',dataIndex: 'status',renderer:function(v){
			var status;
			if(v==-1){
				status = "等待上传";
			}else if(v==-2){
				...
				status =  "<div><img src='theme/js/extjs4.2/src/ux/uploadPanel/icons/ok.png'/></div>";
			}else if(v==-5){
				status =  "停止上传";
			}		
			return status;
		}},
        {
            ...
            width:'9%',
            items: [{
                icon: 'theme/js/extjs4.2/src/ux/uploadPanel/icons/delete.gif',
                tooltip: 'Remove',
...
//                    grid.store.remove(grid.store.getAt(rowIndex));
                	var id = grid.store.getAt(rowIndex).get('id');
                    grid.store.remove(grid.store.getAt(rowIndex));
                    grid.up("panel").swfupload.cancelUpload(id,false);
                }
            }]
        },
        {text: '预览', width:'6%',dataIndex: 'view'}
	...
	upload_success_handler : function(file, serverData, responseReceived){
		...
		var temp = serverData.substring(1,serverData.length-1)
		var data = eval('(' + temp + ')');
		//alert(data.thumbnailImage+""+data.productImage)
		Ext.getCmp('imgUrl').setValue(LOAD_URL+data.productImage) ;
		var viewhtml = "<div><a href='"+LOAD_URL+data.productImage+"' target='blank'>查看</a></div>";
		rec.set('percent', 100);
		rec.set('status', file.filestatus);		
